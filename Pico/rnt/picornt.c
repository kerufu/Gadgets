...
/*
rcsq: parameter of raw detections
	r: row number
	c: column number
	s: scale ratio of image
	q: MAYBE confidence
maxndetections: number of raw detections, most of them are not reliable, so this varialble should be much larger than final required number
cascade: the model from file
angel: rotation of detector
pixels: the input image
nrows, ncols: shape of input image
ldim: FOR CONVINIENCE, set it equal to ncols
scalefactor: the factor to scale up image each time
stridefactor: the speed of scanning of detector
minsize, maxsize: range of detector window size
*/
//return: number of new raw detections
{
	float s;
	int ndetections;
...
/*
n: number of raw detection to process
*/
{
	int idx, ncc, cc;
	int a[4096];
...
/*
slot: the memory slot to update this time
memory: the memory block
counts: number of raw detection for the memory slot to update
maxslotsize: raw detections outside of this limit will be overwrite in next update. FOR CONVINIENCE, set it half of maxndets
*/
//return: number of all raw detections in memory
{
	int i, j;
	...
	*slot = (*slot + 1)%nmemslots;
	...
	//read all stored raw detection, until maxndets. memory slot index is priority
	ndets = 0;
